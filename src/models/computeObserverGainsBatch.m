...
ns_ROM = 2:2:20;
ns_ROM_lopinf = 2:2:20;

%% Iterate through each rom dimension
n_train=3;
for i = ns_ROM
    % generate file names for each rom
    eraFile = romDir + sprintf("eraSystemMatrices_%ddim_1train.mat",i);
    dmdcFile = romDir + sprintf("dmdcSystemMatrices_%ddim_%dtrain.mat",i,n_train);
    % Load in files
    load(eraFile);
    load(dmdcFile);
    ...
    newEraFile = newRomDir + sprintf("eraSystemMatricesAndGains_%ddim_1train.mat",i);
    newDmdcFile = newRomDir + sprintf("dmdcSystemMatricesAndGains_%ddim_%dtrain.mat",i,n_train);
...
for i = ns_ROM_lopinf
    % generate file names for each rom
    lopinfFile = romDir + sprintf("lopinfSystemMatrices_%ddim_%dtrain.mat",i,n_train);
    % Load in files
    load(lopinfFile);
    ...
    % Save new files with observer gains
    newLopinfFile = newRomDir + sprintf("lopinfSystemMatricesAndGains_%ddim_%dtrain.mat",i,n_train);
