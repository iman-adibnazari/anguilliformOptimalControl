    ...
    for ref_a_max in ref_a_maxs:
        for ref_omega in ref_omegas:
            for ref_k in ref_ks:
                ...
                trial_name = f"BioinspiredControlTrial_{romName}_{ref_a_max}_{ref_omega}_{ref_k}"
                ...
                if USE_GUI:
                    if isTrainingTrial:
                        ...
                        description = f"BioinspiredControlTrial: Model-{romName}, refTrajParams: a_max-{ref_a_max}, omega-{ref_omega}, k-{ref_k}, timeHorizon-{TimeHorizon}" #f"dt: {dt}, amplitudes: {amplitudes}, frequencies: {frequencies}, phases: {phases}, numSteps: {numSteps}"
                else:
                    if isTrainingTrial:
                        description = f"ControlTrial: Model-{romName}, refTrajParams: feasibleTrajectoryTrialNum-{trainingTrialInd}"
                    else:
